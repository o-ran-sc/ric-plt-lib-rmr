...
mrc = rmr.rmr_init( listen_port, rmr.RMR_MAX_RCV_BYTES, rmr.RMFL_MTCALL )    # put into multi-threaded call mode
...
while True:
    ...
    #mbunch = stretch.rmr_rcvall_msgs( mrc, [2, 4, 6] )     # get types 2, 4 and 6 only
    #mbunch = stretch.rmr_rcvall_msgs( mrc, 2 )             # get only type 2
    mbunch = stretch.rmr_rcvall_msgs( mrc )                 # get all message types
