...
def test_meid():
    ...
    rmr.rmr_set_meid(sbuf, b"\x01\x02")
    assert (
        rmr.rmr_get_meid(sbuf) == rmr.message_summary(sbuf)["meid"] == b"\x01\x02\x00" + b"6" * 29
    )
    assert len(rmr.rmr_get_meid(sbuf)) == 31

    # test that an exception is raised for buffers which are too long
    ex_raised = False
    try:
        rmr.rmr_set_meid(sbuf, b"8" * 32)
    except exceptions.MeidSizeOutOfRange as err:
        ex_raised = True
