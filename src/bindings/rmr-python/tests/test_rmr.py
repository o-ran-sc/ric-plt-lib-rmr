...
def test_send_rcv_subid():
    ...
    assert send_summary["message state"] == rcv_summary["message status"] == "RMR_OK"
    assert pre_send_summary["payload"] == rcv_summary["payload"] == pay
    assert pre_send_summary["message type"] == rcv_summary["message type"] == 46656
    assert pre_send_summary["subscription id"] == rcv_summary["subscription id"] == 777
